...
private slots:
	void initTestCase() { QVERIFY(server.listen(QHostAddress::LocalHost, 4571)); }
...
private slots:
	void should_send_valid_requests_data()
	{
		...
		QTest::newRow("Simple GET") << QByteArray("GET") << QUrl("http://127.0.0.1:4571/") << Pillow::HttpHeaderCollection() << QByteArray()
		                            << HttpRequestData()
		                                   .withMethod("GET")
		                                   .withUri("/")
		                                   .withPath("/")
		...
		QTest::newRow("GET with headers") << QByteArray("GET") << QUrl("http://127.0.0.1:4571/some/path?and=query")
		...
		QTest::newRow("GET with connection: close")
		    << QByteArray("GET") << QUrl("http://127.0.0.1:4571/")
		...
		QTest::newRow("Simple PUT") << QByteArray("PUT") << QUrl("http://127.0.0.1:4571/some/path") << (Pillow::HttpHeaderCollection())
		...
		QTest::newRow("Large POST") << QByteArray("POST") << QUrl("http://127.0.0.1:4571/some/large/path")
	...
	void should_send_valid_requests()
	{
		...
		QBuffer requestContent(&content);
		requestContent.open(QIODevice::ReadOnly);
	...
	void should_receive_response()
	{
		...
		QVERIFY(server.waitForRequest());
		server.receivedConnections.last()->writeResponse(201, Pillow::HttpHeaderCollection() << Pillow::HttpHeader("X-Some", "Header"),
		                                                 "Hello World!");
		...
		QCOMPARE(r->rawHeaderPairs(), Pillow::HttpHeaderCollection()
		                                  << Pillow::HttpHeader("x-some", "Header") << Pillow::HttpHeader("content-length", "12")
	...
	void should_set_cooked_headers_from_received_headers()
	{
		...
		server.receivedConnections.last()->writeResponse(
		    302,
		    Pillow::HttpHeaderCollection() << Pillow::HttpHeader("Location", "http://example.org")
		                                   << Pillow::HttpHeader("Content-Type", "some/type")
		                                   << Pillow::HttpHeader("Last-Modified", Pillow::HttpProtocol::Dates::getHttpDate(QDateTime(
		                                                                              QDate(2012, 4, 30), QTime(8, 9, 0), QTimeZone::UTC)))
		...
		QCOMPARE(r->rawHeaderPairs(), Pillow::HttpHeaderCollection()
		                                  ...
		                                  << Pillow::HttpHeader("content-length", "6") << Pillow::HttpHeader("content-type", "some/type")
		                                  << Pillow::HttpHeader("set-cookie", "ChocolateCookie=Very Delicious"));
		...
		QCOMPARE(r->header(QNetworkRequest::LastModifiedHeader).toDateTime(),
		         QDateTime(QDate(2012, 4, 30), QTime(8, 9, 0), QTimeZone::UTC));
	...
	void should_set_headers_as_soon_as_they_are_received()
	{
		QNetworkReply* r = nam->get(QNetworkRequest(testUrl()));
		QVERIFY(server.waitForRequest());
		server.receivedConnections.last()->writeHeaders(
		    200, Pillow::HttpHeaderCollection() << Pillow::HttpHeader("X-Some", "Header") << Pillow::HttpHeader("Content-Length", "12")
		                                        << Pillow::HttpHeader("Content-Type", "text/plain"));
